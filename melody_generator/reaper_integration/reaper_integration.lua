...
-- Generates a melodic MIDI sequence with exact note count
function M.create_track(root_note, octave, scale_notes, note_length)
    beats_per_measure = beats_per_measure or 4
    note_length = note_length or 16
    
    reaper.InsertTrackAtIndex(0, true)
    local track = reaper.GetTrack(0, 0)
    
    -- Create MIDI item (expandable)
    local measures = 8
    local start_pos = 0
    local end_pos = measures * beats_per_measure
    local item = reaper.CreateNewMIDIItemInProj(track, start_pos, end_pos, false)
    ...
    -- weighted durations for variety
    local durations = {0.25, 0.5, 1, 1.5}  -- sixteenth, eighth, quarter, dotted quarter
    local weights   = {0.15, 0.35, 0.4, 0.1}
    ...
    while notes_added < note_length do
        -- 15% chance of a rest between notes
        if math.random() < 0.15 and notes_added > 0 then
            pos = pos + pick_weighted({0.25, 0.5}, {0.5, 0.5})  -- short rests
        end
        
        -- melodic movement with occasional leaps
        local step
        if math.random() < 0.1 then
            -- occasional leap
            step = math.random(1, #scale_notes)
        ...
        -- velocity: accented on strong beats
        local beat_pos = pos % 1
        local velocity = beat_pos == 0 and 110 or 85
    ...
    -- Resize item to fit the actual content
    reaper.SetMediaItemLength(item, pos, false)
    
    reaper.MIDI_Sort(take)
    reaper.UpdateArrange()
    
    reaper.ShowConsoleMsg("Generated " .. note_length .. " notes\n")
end
